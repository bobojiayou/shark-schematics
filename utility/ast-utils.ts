...
export function insertImport(source: ts.SourceFile, fileToEdit: string, symbolName: string,
  ...
  if (relevantImports.length > 0) {
    ...
    if (importTextNodes.length === 0) {
      ...
      return insertAfterLastOccurrence(imports, `, ${symbolName}`, fileToEdit, fallbackPos, source);
    }
...
export function insertAfterLastOccurrence(nodes: ts.Node[],
  ...
  // 纠正
  const correctLen = getChineseCharacterNum(source.text, 0, (lastItem as any).end)

  const lastItemPosition: number = lastItem ? lastItem.getEnd() + correctLen : fallbackPos + correctLen;
...
export function addSymbolToNgModuleMetadata(
  ...
  // position纠正
  const correctLen = getChineseCharacterNum(source.text, 0, node.getEnd());
  position += correctLen;
  if (importPath !== null) {
    return [
      new InsertChange(ngModulePath, position, toInsert),
...
/**  
 * Count the chinese characters in range filed
*/

export function getChineseCharacterNum(source: any, start: any, end: any) {
  if (!source) {
    return 0;
  }
